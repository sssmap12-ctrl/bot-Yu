...
# Load environment variables (TOKEN, WEBHOOK_URL)
load_dotenv()
...
# Initialize bot and dispatcher (no polling, using webhook)
bot = Bot(token=TOKEN)
dp = Dispatcher()

def get_ydl_opts(output_path: str) -> dict:
    """Return yt-dlp options for audio extraction to MP3 with thumbnail embed."""
    return {
...
# Register the webhook with Vercel (this will be called from the server entry‑point)
async def on_startup(_: Bot) -> None:
    # Ensure webhook is set when the app boots (idempotent)
    webhook_url = f"{WEBHOOK_URL}"  # Full URL to the Vercel endpoint (e.g., https://myproj.vercel.app/api/webhook)
    await bot.set_webhook(webhook_url)

# The dispatcher must be started by the FastAPI app (see api/main.py)
